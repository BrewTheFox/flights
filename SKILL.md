...
*device information is stored in this file:*  `~/.config/flights/lightbulbs.json`.

Each device contains the following fields: 
```json
{
    name (set by the user).
    key (Tuya local key. The length of this field MUST always be equal 16).
...
- To add a lightbulb you can use the command `flights add --name {name provided by user} --key {escaped key} --id {device id} --ip {device ip address}`
